...
**5. Client Components (`"use client"`) are allowed only for interactive UI.**
The two authorised `"use client"` components are `WizardStepper.tsx` (real-time CFM calculations, 60 fps counters, dynamic PDF rendering) and `CatalogView.tsx` (interactive product filtering and autocomplete). All other pages and layouts are React Server Components by default.
...
│   │   ├── catalog-chat.ts       # AI-free catalog assistant
