...
sidebarTitle: "RBAC"
description: "B2B Import ERP enforces access through 9 predefined roles. Permission checks cascade from role assignment through user exceptions and site-level restrictions."
---
...
## Full System Role Catalogue

The platform defines 26 role codes in total (used for custom role creation and fine-grained staffing hierarchies). The nine documented above are the **active roles** shipped by default. The complete catalogue includes:

`SUPER_ADMIN` · `ADMIN_EMPRESA` · `GERENTE_GENERAL` · `DIRECTOR_OPERACIONES` · `DIRECTOR_COMERCIAL` · `DIRECTOR_FINANCIERO` · `JEFE_PROYECTOS` · `JEFE_MANTENIMIENTO` · `JEFE_INVENTARIO` · `JEFE_COMPRAS` · `JEFE_CARTERA` · `JEFE_CALIDAD` · `EJECUTIVO_COMERCIAL` · `INGENIERO_COMERCIAL` · `INGENIERO_PROYECTOS` · `COORDINADOR_OPERACIONES` · `TECNICO_CAMPO` · `ALMACENISTA` · `AUXILIAR_INVENTARIO` · `AUXILIAR_CARTERA` · `AUXILIAR_ADMINISTRATIVO` · `CONTADOR` · `AUDITOR` · `CLIENTE` · `PROVEEDOR` · `INVITADO`

Custom roles created within a tenant have a non-`NULL` `tenant_id` in the `roles` table. System-level global roles carry `tenant_id = NULL`.
