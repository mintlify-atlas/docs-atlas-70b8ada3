---
title: "Inventory Module: SKU Catalog, Stock Movements, and Purchases"
sidebarTitle: "Inventory"
description: "Manage product SKUs, track stock levels with full movement history, create purchase orders, and link inventory consumption to work orders."
---
...
| `Ajuste` | Positive or negative stock correction | Restricted to `JEFE_INVENTARIO` / Gerencia |
...
| Permission | Roles |
|---|---|
| `items.manage` (catalog CRUD) | `JEFE_INVENTARIO`, `GERENTE`, `GERENTE_GENERAL` |
| `warehouses.manage` | `JEFE_INVENTARIO`, `GERENTE`, `GERENTE_GENERAL` |
| `inventory.movement` (Entrada, Salida, Reserva) | `ALMACENISTA`, `JEFE_INVENTARIO`, `GERENTE`, `GERENTE_GENERAL` |
| `inventory.adjustment` (Ajuste, Transferencia) | `JEFE_INVENTARIO`, `GERENTE`, `GERENTE_GENERAL` |
| `inventory.approve` (Registrado → Aplicado) | `JEFE_INVENTARIO`, `GERENTE`, `GERENTE_GENERAL` |
| `inventory.view` | All internal roles; `AUDITOR` can see soft-deleted records |
