...
| `warranties.manage` (create, edit) | `GERENTE`, `GERENTE_GENERAL`, `JEFE_PROYECTOS`, `COORDINADOR_OPERACIONES` |
| `interventions.manage` | `GERENTE`, `GERENTE_GENERAL`, `JEFE_PROYECTOS`, operations staff |
| `warranties.cancel` | `GERENTE`, `GERENTE_GENERAL` (requires `cancel_reason`) |

All `warranties` and `warranty_interventions` rows have RLS enabled and are scoped to `tenant_id`. The Auditor role can view soft-deleted records for compliance purposes.
